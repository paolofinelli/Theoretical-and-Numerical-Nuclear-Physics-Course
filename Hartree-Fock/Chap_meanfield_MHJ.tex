...
{Nuclear Many-body Physics \quad \quad - a Computational Approach - Mean-field Approaches}
...
\]
where we have skipped the dependence on $x,y,z$ and introduced the shorthand $\psi_x$, $\psi_y$ and $\psi_z$  for the various derivatives. For $\psi^*$ the Euler-Lagrange  equations yield
\[
...
This means that our Hamiltonian is written as the sum of some one-body part and 
a two-body part
\begin{equation}
...
\begin{eqnarray}
\epsilon_{\mu} \psi_{\mu}(x_i) & = & \left[ -\frac{1}{2}\nabla_i^2+ \sum_{\nu=1}^A\int \psi_{\nu}^*(x_j)\hat{v}(r_{ij})\psi_{\nu}(x_j)~dx_j \right]\psi_{\mu}(x_i) \nonumber\\ & - & \left[ \sum_{{\nu}=1}^A \int\psi_{\nu}^*(x_j)\hat{v}(r_{ij})\psi_{\mu}(x_j)~dx_j\right] \psi_{\nu}(x_i) \; .  \label{eq:hartreefockcoordinatespace}
\end{eqnarray}
...
\]
and where $\hat{h}_0(i)$ is the one-body part. The latter is normally chosen as a part which yields solutions in closed form. The harmonic oscillator is a classical problem thereof.
...
\sum_{j=1}^A\sum_{\gamma\delta} C^*_{j\gamma}C_{j\delta}\langle \alpha\gamma|\hat{v}|\beta\delta\rangle_{AS}.
\]
The Hartree-Fock equations are solved in an iterative way starting with a guess for the coefficients $C_{j\gamma}=\delta_{j,\gamma}$ and solving the equations by diagonalization till the new single-particle energies
...
\sum_{i\le F}^A \langle i | \hat{h}_0 +\hat{u}^{HF}| j\rangle+ \nonumber\\
& & \frac{1}{2}\sum_{i\le F}^A\sum_{j \le F}^A\left[\langle ij |\hat{v}|ij \rangle-\langle ij|\hat{v}|ji\rangle\right]-\sum_{i\le F}^A \langle i |\hat{u}^{HF}| i\rangle,
\end{eqnarray} 
which results in
\[
  E_0^{HF}
...
\[
  E_0^{HF}
  = \sum_{i\le F}^A \varepsilon_i^{HF} - \frac{1}{2}\sum_{i\le F}^A\sum_{j \le F}^A\left[\langle ij |\hat{v}|ij \rangle-\langle ij|\hat{v}|ji\rangle\right].
\]
...
\[
  E[\Phi^{\mathrm{HF}}(N)]-   E[\Phi^{\mathrm{HF}}(N-1)] = \langle k | \hat{h}_0 | k \rangle +
  %\frac{1}{2}
  \sum_{j=1}^N\langle kj|\hat{v}|kj\rangle_{AS}
\]
...
\[
  ...
  = \langle k | \hat{h}_0 | k \rangle +
  %\frac{1}{2}
  \sum_{j=1}^A\langle kj|\hat{v}|kj\rangle_{AS}
\]
which is just our definition of the Hartree-Fock single-particle energy
\[
  E[\Phi^{\mathrm{HF}}(A)]-   E[\Phi^{\mathrm{HF}}(A-1)] 
...
\section{Stability of HF solution}
...
\]
our starting point. We have shown that our general representation of a Slater determinant is
\[
...
&  & \frac{1}{2!}\sum_{ab>F}
\sum_{ij\le F}\delta C_{ai}\delta C_{bj}\langle c |\hat{H}a_{a}^{\dagger}a_{i}a_{b}^{\dagger}a_{j}|c\rangle+ \nonumber\\ & & \frac{1}{2!}\sum_{ab>F}
...
%\end{align}
resulting for the right-hand part in
\[
...
If we define the two matrix elements
\[
...
we can write out the energy as
\begin{eqnarray}
\langle c'|H|c'\rangle& = & \left(1+\sum_{ai}|\delta C_{ai}|^2\right)\langle c |H|c\rangle+ \nonumber\\& & \sum_{ai}|\delta C_{ai}|^2(\varepsilon_a^{HF}-\varepsilon_i^{HF})+\sum_{ijab}A_{ai,bj}\delta C_{ai}^*\delta C_{bj}\nonumber \\
...
\]
with $\Delta_{ai,bj} = (\varepsilon_a-\varepsilon_i)\delta_{ab}\delta_{ij}$. The condition
\[
...
A necessary (but no sufficient) condition is that the matrix elements (for all $a_i$ )
\[
...
\subsection*{Exercise \thedoconceexercisecounter: Slater determinants}
\addcontentsline{loe}{doconceexercise}{Exercise \thedoconceexercisecounter: Slater determinants}
...
\psi_{\alpha_{j}}|\hat{v}|\psi_{\alpha_{i}}\psi_{\alpha_{j}}\rangle- \right. \\& \left. 
